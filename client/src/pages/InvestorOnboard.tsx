...
export default function InvestorOnboard() {
  ...
  const steps = [
    <div key="0" className="space-y-5">
      <StepHeader step={0} title="Tell us about you." subtitle="Who's doing the investing?" />
      <Field label="Full name" value={name} onChange={setName} placeholder="Sarah Kim" />
    ...
    <div key="1" className="space-y-6">
      <StepHeader step={1} title="Your investor type." subtitle="How do you deploy capital?" />
      <div className="space-y-2">
    ...
    <div key="2" className="space-y-6">
      <StepHeader step={2} title="Cheque & stage." subtitle="Where in the journey do you invest?" />
      <div className="space-y-2">
      ...
      <div className="space-y-2">
        ...
        <MultiPill options={["Idea", "Pre-Product", "Pre-Revenue", "Early Revenue", "Scaling"]} selected={stages}
          onToggle={v => setStages(p => p.includes(v) ? p.filter(x => x !== v) : [...p, v])} />
      </div>
    </div>,

    <div key="3" className="space-y-6">
      <StepHeader step={3} title="Your focus areas." subtitle="What sectors do you back?" />
      <div className="space-y-2">
        <p className="text-xs text-white/40 uppercase tracking-wider">Sectors</p>
        <MultiPill options={["FinTech", "HealthTech", "AI/ML", "SaaS B2B", "Consumer", "Marketplace", "DeepTech", "Other"]} selected={sectors}
    ...
    <div key="4" className="space-y-5">
      <StepHeader step={4} title="Create your account." subtitle="This is where everything gets saved." />
